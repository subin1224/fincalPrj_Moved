...
public class EmailController {
	...
	// 신청하기 폼 에서 전송  registerApt => /aptMail.do
	@RequestMapping("/aptMail.do")
	public String sendByregisterApt(@ModelAttribute EmailVO vo, Model model) {
		//제 이메일임다 ... 테스트 하는 이메일 설정 해줍사
		vo.setReceiveMail("salllena33@gmail.com");
		...
		//구분
		vo.setSubject("HOUSEMUCH 신청메일"); //이메일에서 제목 구분
		message = "<h1>아파트 명 : "+subject+" <br> </h1> <h2>이름 :"+vo.getSenderName()+"<br><br>"+vo.getSenderMail()+"님+<br></h2>" 
				 + "<b>연락처 : </b>"+ vo.getHp() +"<br><br>" +"<h2>내용</h2><br>" + message; 
		vo.setMessage(message);
	
		try {
			emailService.sendMail(vo);
			model.addAttribute("msg", "해당 신청을 확인후 연락 드리겠습니다.");
		}catch(Exception e) {
			logger.info("이메일 발송 실패..");
			e.printStackTrace();
			model.addAttribute("msg", "이메일 발송 실패..");
